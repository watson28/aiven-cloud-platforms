...
type DebouncedSliderProps<D extends React.ElementType, P> = SliderProps<
  D,
  P
> & { delay: number }

function DebouncedSlider<D extends React.ElementType, T>(
  props: DebouncedSliderProps<D, T>
) {
  const { value, onChange, delay: time } = props
  const [internalValue, setInternalValue] = useState<SliderProps['value']>(
    props.defaultValue
  )
  ...
  useEffect(() => {
    const handler = setTimeout(() => {
      if (
        onChange !== undefined &&
        onChangeEventRef.current !== undefined &&
  ...
  return <Slider {...props} value={internalValue} onChange={handleChange} />
}
